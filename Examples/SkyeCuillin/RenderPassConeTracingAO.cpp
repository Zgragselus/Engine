...
RenderPassConeTracingAO::RenderPassConeTracingAO(Engine::D3DRenderer* renderer, unsigned int width, unsigned int height, unsigned int samples) :
	RenderPassFullscreen(renderer, width, height)
{
	mSamples = samples;

	mOcclusionRS = new Engine::RootSignature(renderer->GetDevice(), 4, 1);
	(*mOcclusionRS)[0].InitAsConstantBuffer(0);
	(*mOcclusionRS)[1].InitAsDescriptorTable(1);
	(*mOcclusionRS)[1].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 1);
	(*mOcclusionRS)[2].InitAsDescriptorTable(1);
	(*mOcclusionRS)[2].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);
	(*mOcclusionRS)[3].InitAsDescriptorTable(1);
	(*mOcclusionRS)[3].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 1);
	mOcclusionRS->InitStaticSampler(0, Engine::Sampler::CreateSamplerD3D(Engine::Sampler::Filter::MIN_LINEAR_MAG_LINEAR_MIP_LINEAR,
		...
		0.0f,
		32.0f));
	mOcclusionRS->Finalize(D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);

	mOcclusion = new Engine::D3DShader("../Data/SkyeCuillin/ConeTracingAO.hlsl",
	...
	mOcclusionPS = new Engine::PipelineState(mRenderer->GetDevice(),
		mOcclusionRS,
		mOcclusion,
...
RenderPassConeTracingAO::~RenderPassConeTracingAO()
{
	delete mOcclusionRS;
	delete mOcclusionPS;
	delete mOcclusion;
...
void RenderPassConeTracingAO::Process(Engine::Entity* camera, Engine::DescriptorHeap* heap, Engine::GraphicsContext* context, std::vector<Engine::RenderNode>& nodes)
{
	if (camera->GameObject().Has<Engine::CameraComponent>())
	{
		...
		context->SetPipelineState(mOcclusionPS);
		context->SetRootSignature(mOcclusionRS);
...
void RenderPassConeTracingAO::Resize(unsigned int width, unsigned int height)
{
	mWidth = width;
	mHeight = height;
