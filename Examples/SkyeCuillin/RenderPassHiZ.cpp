#include "RenderPassHiZ.h"

using namespace SkyeCuillin;

RenderPassHiZ::RenderPassHiZ(Engine::D3DRenderer* renderer, unsigned int width, unsigned int height) : RenderPass(renderer, width, height)
{
...
void RenderPassHiZ::Process(Engine::Entity* camera, Engine::DescriptorHeap* heap, Engine::GraphicsContext* context, std::vector<Engine::RenderNode>& nodes, Engine::GpuMappedBuffer* matrices)
{
...
void RenderPassHiZ::Resize(unsigned int width, unsigned int height)
{
	/*mWidth = width;
	mHeight = height;
