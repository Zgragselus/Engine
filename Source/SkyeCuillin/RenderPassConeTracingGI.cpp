...
RenderPassConeTracingGI::RenderPassConeTracingGI(Engine::D3DRenderer* renderer, unsigned int width, unsigned int height, unsigned int samples) :
	RenderPassFullscreen(renderer, width, height)
{
	mSamples = samples;

	mGlobalIllumRS = new Engine::RootSignature(renderer->GetDevice(), 4, 1);
	(*mGlobalIllumRS)[0].InitAsConstantBuffer(0);
	(*mGlobalIllumRS)[1].InitAsDescriptorTable(1);
	(*mGlobalIllumRS)[1].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 1);
	(*mGlobalIllumRS)[2].InitAsDescriptorTable(1);
	(*mGlobalIllumRS)[2].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);
	(*mGlobalIllumRS)[3].InitAsDescriptorTable(1);
	(*mGlobalIllumRS)[3].SetTableRange(0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 1);
	mGlobalIllumRS->InitStaticSampler(0, Engine::Sampler::CreateSamplerD3D(Engine::Sampler::Filter::MIN_LINEAR_MAG_LINEAR_MIP_LINEAR,
		...
		0.0f,
		32.0f));
	mGlobalIllumRS->Finalize(D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	...
	mGlobalIllumPS = new Engine::PipelineState(mRenderer->GetDevice(),
		mGlobalIllumRS,
		mGlobalIllum,
...
RenderPassConeTracingGI::~RenderPassConeTracingGI()
{
	delete mGlobalIllumRS;
	delete mGlobalIllumPS;
	delete mGlobalIllum;
...
void RenderPassConeTracingGI::Process(Engine::Entity* camera, Engine::DescriptorHeap* heap, Engine::GraphicsContext* context, std::vector<Engine::RenderNode>& nodes)
{
	if (camera->GameObject().Has<Engine::CameraComponent>())
	{
		...
		context->SetPipelineState(mGlobalIllumPS);
		context->SetRootSignature(mGlobalIllumRS);
...
void RenderPassConeTracingGI::Resize(unsigned int width, unsigned int height)
{
	mWidth = width;
	mHeight = height;
